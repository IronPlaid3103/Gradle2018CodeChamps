...
import frc.team3103.robot.subsystems.MotionProfiling.PeriodicRunnable;
...
public class MotionProfiling {
	 ...
	 */
	public MotionProfiling(TalonSRX lefttalon, TalonSRX righttalon) {
		 ...
		 */
		_lefttalon.changeMotionControlFramePeriod(25);
		_righttalon.changeMotionControlFramePeriod(25);
	...
	private void startFilling() {
		...
		startFilling(GeneratedMotionProfile.LeftPoints, GeneratedMotionProfile.RightPoints, GeneratedMotionProfile.kNumPoints);
	}
	...
	private void startFilling(double[][] leftProfile, double [][] rightProfile, int totalCnt) {
		...
		_lefttalon.configMotionProfileTrajectoryPeriod(kBaseTrajPeriodMs, kTimeoutMs);
		_righttalon.configMotionProfileTrajectoryPeriod(kBaseTrajPeriodMs, kTimeoutMs);
		...
		for (int i = 0; i < totalCnt; ++i) {
			...
			leftPoint.position = leftPositionRot * kSensorUnitsPerRotation; //Convert Revolutions to Units
			leftPoint.velocity = leftVelocityRPM * kSensorUnitsPerRotation / 600.0; //Convert RPM to Units/100ms
			...
			rightPoint.position = rightPositionRot * kSensorUnitsPerRotation; //Convert Revolutions to Units
			rightPoint.velocity = rightVelocityRPM * kSensorUnitsPerRotation / 600.0; //Convert RPM to Units/100ms
